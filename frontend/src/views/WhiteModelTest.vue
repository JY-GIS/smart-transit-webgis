...
onMounted(async () => { 
    ...
    try {
        ...
        // 加载福田区公交线路
        const futianBusRoutes = await Cesium.GeoJsonDataSource.load(
            'test-data/futian-bus-routes.geojson',
            {
                stroke: Cesium.Color.GOLD.withAlpha(0.95),
                strokeWidth: 2,
                clampToGround: false,
            }
        )
        ...
        for (const entity of futianBusRoutes.entities.values) {
            if (entity.polyline) {
        ...
        viewer.dataSources.add(futianBusRoutes)

        console.info(`福田公交线路加载完成：${futianBusRoutes.entities.values.length} 条`)
